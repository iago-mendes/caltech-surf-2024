...
\begin{document}
  ...
  \section{Introduction}
    ...
    \begin{figure}
      ...
      \end{tikzpicture}
      \caption{\todo{write description}}
      \label{fig:free-data}
    \end{figure}

    \todo{explain XCTS + SKS, introducing the free data and referring to Figure \ref{fig:free-data}}
  ...
  \section{Results}
    ...
    \subsection{Convergence of asymptotic quantities}
      ...
      \begin{figure}
        ...
        \caption{Comparison between different versions of the center-of-mass surface integrals. $[\psi^4 n^i]$ refers to \eq{\eqref{eq:CoM-surf}}, while $[(\psi^4-1) n^i]$ refers to \eq{\eqref{eq:CoM-surf-1}}. \todo{update BBH with results from \eq{\ref{eq:CoM-surf-1}}}}
      ...
      \begin{figure}
        ...
        \caption{Convergence with resolution for $M_\text{ADM}$, $P_\text{ADM}$, and $C_\text{CoM}$. Each subplot shows the difference at each polynomial order $P$ relative to the highest $P$ result for multiple refinement levels $L$. \todo{update BBH surface with results from \eq{\ref{eq:CoM-surf-1}}}}
    ...
    \subsection{Control of BBH parameters}
      ...
      \begin{figure}
        ...
        \caption{Control results for the BBH runs shown in Table \ref{tab:bbh-runs}. The control residuals ${\bf F}({\bf u})$ are shown for each control iteration $k$. Here, $\vec C_\text{CoM}$ and $\vec P_\text{ADM}$ were computed using \eq{\eqref{eq:CoM-surf-1}} and \eq{\eqref{eq:Padm-surf}}. \todo{update with results from \eq{\ref{eq:CoM-surf-1}}}}
